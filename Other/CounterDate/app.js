function initCountdown(parent, to) {
    ...
    let timer;
    if (parent && to) timer = setInterval(countdown, 1000)
    else null;
  ...
  function countdown() {
    ...
    to
      ...
      : console.error("Countdown error: no toCountDate mantioned");
    let now = new Date();

    let totalSeconds = Math.floor((toCountDate - now) / 1000);
    ...
    if (rootElements.length > 0) {
      rootElements.forEach((root) => {
        ...
        } else root.querySelector(".days").style.display = "none";
    ...
    } else console.error("Countdown error: no parent mantioned");
    }
